...
void Server::parseConfig()
{
    ...
    lanIPRange = lanClientIPs[0].substr(0, lanClientIPs[0].find_last_of('.') + 1);
    cout << lanIPRange << endl;
...
static void *handleClient(void *arg)
{
    ...
    while (true)
    {
        ...
        if (bytes_received > 0)
        {
            ...
            if (packet.corrupted || packet.ttl <= 1)
            {
            ...
            if (Count2(server->lanIPRange, src_ip) && Count2(server->lanIPRange, dest_ip))
            {
                int dest_fd = server->ip_fd_map[dest_ip];
                packet.recomputeTTL();
                write(dest_fd, buffer, bytes_received);
            }
            else if (Count2(server->lanIPRange, src_ip) && (!Count2(server->lanIPRange, dest_ip) && dest_ip != server->server_wanIp))
            {
            ...
            else if (!Count2(server->lanIPRange, src_ip) && dest_ip == server->server_wanIp)
            {
